...
public class SteerCamTarget : MonoBehaviour {
	...
	void OnGUI () {
		...
		GUI.Box(new Rect(5,5,360,80),"");
		GUILayout.Space(10);
		GUILayout.Label("    " + FPSmeter.fps.ToString("f2")+"   WSAD/ARROW - move (hold SHIFT to run)");
		GUILayout.Space(-8);
		GUILayout.Label("    Mouse - camera angle  , /. distance from followed target");
		GUILayout.Space(-8); 
		if (soccer_flag) {
			//GUILayout.Label ("    F (hold) - freeze camera, L - shadow on/off, CLICK to kick");
			GUILayout.Label ("    F (hold) - freeze camera, CLICK to kick");
		} else {
		...
		GUILayout.Space(10);
		GUILayout.BeginArea(new Rect (5,90,100,100));
		if (GUILayout.Button("lock cursor", GUILayout.Width(100))) {
			Cursor.lockState=CursorLockMode.Locked;
		}
		GUILayout.EndArea();
		if (logo) GUI.Label (new Rect(Screen.width-220,Screen.height-128,256,128), logo);
	}
