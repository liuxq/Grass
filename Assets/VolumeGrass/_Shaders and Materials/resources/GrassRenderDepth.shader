Shader "GrassRenderDepth" {
	...
	//
    SubShader {
		Tags { "RenderType"="VolumeGrass" "Queue"="Geometry+248"}
        Pass { ColorMask 0 } // we don't need to render anything but depth (nothing for color buffer)
    	//Pass { Color (1,0,0,0) }
    }
    
    SubShader {
		Tags { "RenderType"="Opaque" "Queue"="Geometry+248"}
        Pass { ColorMask 0 } // we don't need to render anything but depth (nothing for color buffer)
    	//Pass { Color (1,0,0,0) }
    }
    ...
    SubShader {
		Tags { "RenderType"="TreeBark" "Queue"="Geometry+248"}
        Pass { ColorMask 0 }
    	//Pass { Color (1,0,0,0) }
    }
    ...
    SubShader {
		Tags { "RenderType"="TreeOpaque" "Queue"="Geometry+248"}
		Pass {
	...
	SubShader {
		Tags { "RenderType"="TransparentCutout" "Queue"="Geometry+249"}
		Pass {
		ColorMask 0 // we don't need to render anything but depth (nothing for color buffer)		
		CGPROGRAM
			#pragma vertex vert
    ...
    SubShader {
		Tags { "RenderType"="Grass" "Queue"="Geometry+249"}
    ...
    SubShader {
		Tags { "RenderType"="GrassBillboard" "Queue"="Geometry+249"}
    	// we need to handle grass animation, so we use existing predefined pass
		// assumig grass won't animate - we could replace below UsePass with some kind of simple TransparentCutout pass (see above)
		Pass {
			Cull Off		
