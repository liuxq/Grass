...
{
    //joystick占用的fingerIndex，目前只保存一个，有必要再改成多个
    public static int JoystickFingerIndex = -1;

    #region private members

    private Vector2[] joystickOldMousePosition = new Vector2[2];
    private int[] joystickTapCount = new int[2];
    private float[] joystickStartActionTime = new float[2];
    private float[] joystickDeltaTime = new float[2];
    private float[] joystickTapeTime = new float[2];
    ...
    public int TouchCount(bool isJoystick)
    {

#if ((UNITY_ANDROID || UNITY_IPHONE || UNITY_WINRT || UNITY_BLACKBERRY) && !UNITY_EDITOR) 
		return getTouchCount(true,isJoystick);
#else
        return getTouchCount(false, isJoystick);
#endif

    }

    private int getTouchCount(bool realTouch, bool isJoystick)
    {
        ...
        if (!isJoystick)
        {
            count = JoystickFingerIndex < 0 ? count : (count - 1);
        }
    ...
    /// <summary>
    /// 如果在处理Joystick，返回实际的Touch
    /// 如果处理的是正常的EasyTouch，返回除_joystickFingerIndex之外的Touch
    /// </summary>
    /// <param name="index"></param>
    /// <param name="isJoystick">是否在处理Joystick</param>
    /// <returns></returns>
    public static Touch GetTouch(int index, bool isJoystick)
    {
        if (isJoystick)
        {
        ...
        {
            if (JoystickFingerIndex < 0)
            {
                return Input.GetTouch(index);
            }
            else
            {
                if (index < JoystickFingerIndex)
                {
                    return Input.GetTouch(index);
                }
                else
                {
                    return Input.GetTouch(index + 1);
                }
            }
        }
    }
    ...
    public Finger GetMouseTouch(int fingerIndex, Finger myFinger, bool isJoystick)
    {
        Vector2[] oldMousePosition = isJoystick ? joystickOldMousePosition : this.oldMousePosition;
        int[] tapCount = isJoystick ? joystickTapCount : this.tapCount;
        float[] startActionTime = isJoystick ? joystickStartActionTime : this.startActionTime;
        float[] deltaTime = isJoystick ? joystickDeltaTime : this.deltaTime;
        float[] tapeTime = isJoystick ? joystickTapeTime : this.tapeTime;
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            finger.tapCount = tapCount[fingerIndex];
            finger.deltaTime = Time.realtimeSinceStartup - deltaTime[fingerIndex];
            finger.deltaTimeFrame = finger.deltaTime;
