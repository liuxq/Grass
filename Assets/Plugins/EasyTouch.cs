...
public class EasyTouch : MonoBehaviour
{
    ...
    public static event TouchStartHandler On_JoystickTouchStart;
    public static event TouchUpHandler On_JoystickTouchUp;
    public static event TouchDownHandler On_JoystickTouchDown;
    public static event SimpleTapHandler On_JoystickSimpleTap;
    public static event DoubleTapHandler On_JoystickDoubleTap;
    ...
    private float _startTimeAction;									// The time of onset of action.
    private float _joystickStartTimeAction;

    private Finger[] fingers = new Finger[10];						// The informations of the touch for finger 1.
    private Finger[] joystickFingers = new Finger[10];						// The informations of the touch for finger 1.
    ...
    void Update()
    {
        ...
        {
            /*      
             * FIXED by qiulei,[2014-11-8]
             * 修改EasyTouch：
             * 
             * 目的:原版的EasyTouch和Joystick是并发的，只要Joystick接受到手指事件，EasyTouch也同样会接受到，
             * 现需要将Joystick接受到的手指事件忽略掉，只要Joystick接受到手指事件，EasyTouch就不会接受到此手指的信息
             * 
             * 修改内容：
             * 由于Joystick的事件依赖于单指手势，为了尽量减小对EasyTouch的修改，在接收到手指事件后，创建两次单指手势
             * 第一次创建手势为Joystick提供接口，第二次创建屏蔽了Joystick手指信息后的EasyTouch手势
             * 
             */

            //第一次创建单指手势，先处理joystick
            UpdateJoystick();

            //第二次创建屏蔽joystick手指后的单指和双指手势
            UpdateEasyTouch();
        }
    }
    ...
    {
        int joystickCount = input.TouchCount(true);

#if (((UNITY_ANDROID || UNITY_IPHONE || UNITY_WINRT || UNITY_BLACKBERRY) && !UNITY_EDITOR))
				UpdateTouches(true, joystickCount, true);
#else
        UpdateTouches(false, joystickCount, true);
#endif
        // Other fingers gesture
        for (int i = 0; i < 10; i++)
        {
            if (joystickFingers[i] != null)
            {
                OneFinger(i, true);
            }
        }
    ...
    private void UpdateEasyTouch()
    {
        // How many finger do we have ?
        int count = input.TouchCount(false);
        ...
        // Get touches		
#if (((UNITY_ANDROID || UNITY_IPHONE || UNITY_WINRT || UNITY_BLACKBERRY) && !UNITY_EDITOR))
        UpdateTouches(true, count, false);
#else
        UpdateTouches(false, count, false);
#endif
    ...
    void UpdateTouches(bool realTouch, int touchCount, bool isJoystick)
    {
        Finger[] fingers = isJoystick ? joystickFingers : this.fingers;
        ...
        {
            ResetTouches(isJoystick);
            for (var i = 0; i < touchCount; ++i)
            {
                if (fingers == null || i >= fingers.Length)
                {
                    return;
                }

                Touch touch = EasyTouchInput.GetTouch(i, isJoystick);
                ...
                fingers[i].deltaTimeFrame = Time.deltaTime;
        ...
        else
        {
            ...
            while (i < touchCount)
            {
                fingers[i] = input.GetMouseTouch(i, fingers[i], isJoystick) as Finger;
                fingers[i].touchCount = touchCount;
                i++;
            }
        }
    ...
    void ResetTouches(bool isJoystick)
    {
        Finger[] fingers = isJoystick ? joystickFingers : this.fingers;
    ...
    private void OneFinger(int fingerIndex, bool isJoystick)
    {
        Finger[] fingers = isJoystick ? joystickFingers : this.fingers;
        ...
        if (fingers[fingerIndex].gesture == GestureType.None)
        {
            if (isJoystick)
            {
                _joystickStartTimeAction = Time.realtimeSinceStartup;
            }
            else
            {
                _startTimeAction = Time.realtimeSinceStartup;
            }
            ...
            // we notify a touch
            CreateGesture(fingerIndex, EventName.On_TouchStart, fingers[fingerIndex], 0, SwipeType.None, 0, Vector2.zero, isJoystick);
        }

        // Calculates the time since the beginning of the action.
        if (isJoystick)
        {
            timeSinceStartAction = Time.realtimeSinceStartup - _joystickStartTimeAction;
        }
        else
        {
            timeSinceStartAction = Time.realtimeSinceStartup - _startTimeAction;
        }
        ...
        if (fingers[fingerIndex].phase != TouchPhase.Ended && fingers[fingerIndex].phase != TouchPhase.Canceled)
        {
            ...
            if (fingers[fingerIndex].phase == TouchPhase.Stationary && timeSinceStartAction >= longTapTime && fingers[fingerIndex].gesture == GestureType.Acquisition)
            {
                ...
                CreateGesture(fingerIndex, EventName.On_LongTapStart, fingers[fingerIndex], timeSinceStartAction, SwipeType.None, 0, Vector2.zero, isJoystick);
            }
            ...
            if ((fingers[fingerIndex].gesture == GestureType.Acquisition || fingers[fingerIndex].gesture == GestureType.LongTap) && (FingerInTolerance(fingers[fingerIndex]) == false))
            {
                ...
                {
                    fingers[fingerIndex].gesture = GestureType.Cancel;
                    CreateGesture(fingerIndex, EventName.On_LongTapEnd, fingers[fingerIndex], timeSinceStartAction, SwipeType.None, 0, Vector2.zero, isJoystick);
                ...
                else
                {
                    ...
                    if (fingers[fingerIndex].pickedObject)
                    {
                        ...
                        CreateGesture(fingerIndex, EventName.On_DragStart, fingers[fingerIndex], timeSinceStartAction, SwipeType.None, 0, Vector2.zero, isJoystick);
                    }
                    // If not swipe
                    else
                    {
                        fingers[fingerIndex].gesture = GestureType.Swipe;
                        CreateGesture(fingerIndex, EventName.On_SwipeStart, fingers[fingerIndex], timeSinceStartAction, SwipeType.None, 0, Vector2.zero, isJoystick);
                    }
                }
            }
            ...
            if (message != EventName.None)
            {
                ...
                CreateGesture(fingerIndex, message, fingers[fingerIndex], timeSinceStartAction, currentSwipe, 0, fingers[fingerIndex].deltaPosition, isJoystick);
            }

            // TouchDown
            CreateGesture(fingerIndex, EventName.On_TouchDown, fingers[fingerIndex], timeSinceStartAction, currentSwipe, 0, fingers[fingerIndex].deltaPosition, isJoystick);
        }
            ...
            switch (fingers[fingerIndex].gesture)
            {
                ...
                case GestureType.Acquisition:
                    if (FingerInTolerance(fingers[fingerIndex]))
                    {
                        ...
                        {
                            CreateGesture(fingerIndex, EventName.On_SimpleTap, fingers[fingerIndex], timeSinceStartAction, SwipeType.None, 0, Vector2.zero, isJoystick);
                        }
                        else
                        {
                            CreateGesture(fingerIndex, EventName.On_DoubleTap, fingers[fingerIndex], timeSinceStartAction, SwipeType.None, 0, Vector2.zero, isJoystick);
                        }
                    ...
                    else
                    {
                        ...
                        {
                            CreateGesture(fingerIndex, EventName.On_DragStart, fingers[fingerIndex], timeSinceStartAction, SwipeType.None, 0, Vector2.zero, isJoystick);
                            CreateGesture(fingerIndex, EventName.On_Drag, fingers[fingerIndex], timeSinceStartAction, currentSwipe, 0, fingers[fingerIndex].deltaPosition, isJoystick);
                            CreateGesture(fingerIndex, EventName.On_DragEnd, fingers[fingerIndex], timeSinceStartAction, GetSwipe(fingers[fingerIndex].startPosition, fingers[fingerIndex].position), (fingers[fingerIndex].startPosition - fingers[fingerIndex].position).magnitude, fingers[fingerIndex].position - fingers[fingerIndex].startPosition, isJoystick);
                        ...
                        {
                            CreateGesture(fingerIndex, EventName.On_SwipeStart, fingers[fingerIndex], timeSinceStartAction, SwipeType.None, 0, Vector2.zero, isJoystick);
                            CreateGesture(fingerIndex, EventName.On_Swipe, fingers[fingerIndex], timeSinceStartAction, currentSwipe, 0, fingers[fingerIndex].deltaPosition, isJoystick);
                            CreateGesture(fingerIndex, EventName.On_SwipeEnd, fingers[fingerIndex], timeSinceStartAction, GetSwipe(fingers[fingerIndex].startPosition, fingers[fingerIndex].position), (fingers[fingerIndex].position - fingers[fingerIndex].startPosition).magnitude, fingers[fingerIndex].position - fingers[fingerIndex].startPosition, isJoystick);
                ...
                case GestureType.Cancel:
                    CreateGesture(fingerIndex, EventName.On_Cancel, fingers[fingerIndex], 0, SwipeType.None, 0, Vector2.zero, isJoystick);
                    break;
            }

            if (realEnd)
            {
                CreateGesture(fingerIndex, EventName.On_TouchUp, fingers[fingerIndex], timeSinceStartAction, SwipeType.None, 0, Vector2.zero, isJoystick);
    ...
    private void CreateGesture(int touchIndex, EventName message, Finger finger, float actionTime, SwipeType swipe, float swipeLength, Vector2 swipeVector, bool isJoystick)
    {
        Finger[] fingers = isJoystick ? joystickFingers : this.fingers;

        if (fingers == null || fingers[touchIndex] == null)
        {
            return;
        }
        ...
        if (message == EventName.On_TouchStart || message == EventName.On_TouchUp)
        {
            ...
            isStartHoverNGUI = IsTouchHoverNGui(touchIndex, isJoystick, out hitGameObject);
            fingers[touchIndex].isHoverOnNGUI = isStartHoverNGUI;
            fingers[touchIndex].pickedObject = hitGameObject;
        }
        ...
        gesture.deltaTimeFrame = finger.deltaTimeFrame;
        ...
        gesture.isHoverReservedArea = IsTouchReservedArea(touchIndex, isJoystick);

        //Fixed by qiulei,[2014-11-6]，不应该碰到NGUI就把手势屏蔽掉，应该把这件事告诉手势，因此在手势里添加字段isHoverOnNGUI
        gesture.isHoverOnNGUI = fingers[touchIndex].isHoverOnNGUI;
        ...
        {
            RaiseEvent(message, gesture, isJoystick);
        }
    ...
    {
        if (fingers == null || fingers.Length < 2 || fingers[twoFinger0] == null || fingers[twoFinger1] == null)
        {
            return;
        }
        ...
        {
            GameObject hitGameObject;
            isStartHoverNGUI = fingers[twoFinger0].isHoverOnNGUI & IsTouchHoverNGui(twoFinger1, false, out hitGameObject);
        }

        fingers[twoFinger0].isHoverOnNGUI = IsTouchHoverNGui(twoFinger0, false, out fingers[twoFinger0].pickedObject);
        fingers[twoFinger1].isHoverOnNGUI = IsTouchHoverNGui(twoFinger1, false, out fingers[twoFinger1].pickedObject);
        ...
        if (fingers[twoFinger0] != null)
        {
            ...
            gesture.deltaTimeFrame = fingers[twoFinger0].deltaTimeFrame;
        }
        else if (fingers[twoFinger1] != null)
        {
            gesture.deltaTime = fingers[twoFinger1].deltaTime;
            gesture.deltaTimeFrame = fingers[twoFinger1].deltaTimeFrame;
        }
        else
        {
            gesture.deltaTime = 0;
            gesture.deltaTimeFrame = 0;
        }
        ...
        if (fingers[twoFinger0] != null)
        {
            gesture.isHoverReservedArea = IsTouchReservedArea(twoFinger0, false);
        }
        if (fingers[twoFinger1] != null)
        {
            gesture.isHoverReservedArea = gesture.isHoverReservedArea || IsTouchReservedArea(twoFinger1, false);
        }

        //Fixed by qiulei,[2014-11-6]，不应该碰到NGUI就把手势屏蔽掉，应该把这件事告诉手势，因此在手势里添加字段isHoverOnNGUI
        gesture.isHoverOnNGUI = fingers[twoFinger0].isHoverOnNGUI | fingers[twoFinger1].isHoverOnNGUI;
        ...
        {
            RaiseEvent(message, gesture, false);
        }
    }
    ...
    private void RaiseEvent(EventName evnt, Gesture gesture, bool isJoysticEvent)
    {
        if (isJoysticEvent)
        {
            switch (evnt)
            {
                case EventName.On_TouchStart:
                    if (On_JoystickTouchStart != null)
                        On_JoystickTouchStart(gesture);
                    break;
                case EventName.On_TouchDown:
                    if (On_JoystickTouchDown != null)
                        On_JoystickTouchDown(gesture);
                    break;
                case EventName.On_TouchUp:
                    if (On_JoystickTouchUp != null)
                        On_JoystickTouchUp(gesture);
                    break;
                case EventName.On_SimpleTap:
                    if (On_JoystickSimpleTap != null)
                        On_JoystickSimpleTap(gesture);
                    break;
                case EventName.On_DoubleTap:
                    if (On_JoystickDoubleTap != null)
                        On_JoystickDoubleTap(gesture);
                    break;
            }
    ...
    private bool IsTouchHoverNGui(int touchIndex, bool isJoystick, out GameObject hitGameObject)
    {
        Finger[] fingers = isJoystick ? joystickFingers : this.fingers;
        ...
        if (enabledNGuiMode)
        {
            ...
            {
                if (nGUICameras[i] == null || fingers == null || fingers.Length <= touchIndex || fingers[touchIndex] == null)
                {
                    return false;
                }

                if (nGUICameras[i].gameObject.activeSelf)
                {
    ...
    /// 是否接触到保留区
    /// </summary>
    /// <param name="touchIndex">this.fingers数组的下标，不是finger.fingerIndex成员</param>
    /// <param name="isJoystick"></param>
    /// <returns></returns>
    private bool IsTouchReservedArea(int touchIndex, bool isJoystick)
    {
        Finger[] fingers = isJoystick ? joystickFingers : this.fingers;
    ...
    public static void SetEnabled(bool enable)
    {
        ...
        if (enable)
        {
            EasyTouch.instance.ResetTouches(true);
            EasyTouch.instance.ResetTouches(false);
        }
    }
    ...
    /// int
    /// </returns>
    public static int GetTouchCount(bool isJoystick = false)
    {
        if (EasyTouch.instance != null)
        {
            return EasyTouch.instance.input.TouchCount(isJoystick);
        }
        else
        {
            return 0;
